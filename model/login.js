...
const Login = createModel("Login");

const getLoginData = async (req, res) => {
  try {
    const data = await Login.find(); // Retrieve all documents from the 'menu' collection
...
const saveSelectedText = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while saving selected texts" });
  }
};

router.get("/Logindata", getLoginData);
